    ...
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            if user_info_form.is_valid():
                ...
                employment = user_info_form.cleaned_data['employment']
                location = user_info_form.cleaned_data['location']
                interest = user_info_form.cleaned_data['interests']
...
def people_view(request):
    ...
    if request.user.is_authenticated:
        ...
        if not request.session.get('counter'):
            all_people = models.UserInfo.objects.exclude(user=request.user).filter()[:1]
        else:
            view_more = request.session['counter']
            all_people = models.UserInfo.objects.exclude(user=request.user).filter()[:view_more]


        # TODO Objective 5: create a list of all friend requests to current user
        try:
            friend_requests = models.FriendRequest.objects.filter(to_user=request.user.id)
        except:
            friend_requests = []
    ...
    if frID is not None:
        ...
        if request.user.is_authenticated:
            if request.method =='POST':
                ...
                friend_user = models.User.objects.get(id=frID)
                to_user = models.UserInfo.objects.get(user=friend_user)
                frequest = models.FriendRequest.objects.get_or_create(from_user=from_user,to_user=to_user)
